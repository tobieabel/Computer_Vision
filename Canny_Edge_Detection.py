#Use canny edge detection and Hough Transform algorithms to detect circles
#Taken from below website which has very good explanation of what the algorithms are doing
...
def filter_strong_edges(gradient, width, height, low, high):#increase both high and low parameters to filter out more noise
    # Keep strong edges
    keep = set()
    for x in range(width):
        for y in range(height):
    ...
    # Keep weak edges if they are next to a strong pixel in keep()
    lastiter = keep
    while lastiter:
        newkeep = set()
        for x, y in lastiter:
...
if __name__ == "__main__":
    ...
    input_image = Image.open("/Users/tobieabel/Desktop/Ball/Staticball173313.jpg")
